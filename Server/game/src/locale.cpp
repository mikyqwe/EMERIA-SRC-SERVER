...
#include "char.h"
typedef std::map< std::string, std::string > LocaleStringMapTypeNew;
...
const char * locale_find_new(BYTE language, const char *string)
{
	...
	LocaleStringMapTypeNew::const_iterator iter = localeStringNew[language].find( string );

	if( iter == localeStringNew[language].end() )
	{
		static char s_line[1024] = "@0949";
		strlcpy(s_line + 5, string, sizeof(s_line) - 5);
...
void locale_add_new(BYTE language, const char **strings)
{
	LocaleStringMapTypeNew::const_iterator iter = localeStringNew[language].find(strings[0]);

	if( iter == localeStringNew[language].end() )
	{
		localeStringNew[language].insert(std::make_pair(strings[0], strings[1]));	
	}
}
...
void locale_init_new(BYTE language, const char *filename)
{
	...
	do
	{
		...
		if (*tmp == '"')
		{
			...
			locale_add_new(language, (const char**)strings);
