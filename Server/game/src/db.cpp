...
void DBManager::SendAuthLogin(LPDESC d)
{
	...
	strlcpy(ptod.szLanguage, r.language, sizeof(ptod.szLanguage));
	#endif
					...
					{
						{
							...
							char szQuery[1024];
							#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
							snprintf(szQuery, sizeof(szQuery), "UPDATE account SET last_play=NOW(), language='%s' WHERE id=%u", pinfo->language, dwID);
							#else
						...
						#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
						strlcpy(r.language, pinfo->language, sizeof(r.language));
						#endif
