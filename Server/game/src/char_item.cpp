		...
		if ((item2 = GetItem(DestCell))) {
			if (item2->GetVnum() == 55002) {
				...
				if(item2->GetAttributeValue(0) > 0){
					...
					std::unique_ptr<SQLMsg> pEvo(DBManager::instance().DirectQuery("SELECT evolution FROM new_petsystem WHERE id = %d", item->GetID()));
...
/*         if(GetLevel() < item->GetAttributeValue(4))
		...
		//fixing memory leak
#ifdef ENABLE_FIX_PET_TRANSPORT_BOX
		DBManager::instance().DirectQuery("UPDATE new_petsystem SET id =%d WHERE id = %d", item2->GetID(), item->GetSocket(3) != 0 ? item->GetSocket(3) : item->GetID()));
#else
		DBManager::instance().DirectQuery("UPDATE new_petsystem SET id =%d WHERE id = %d", item2->GetID(), item->GetID());
#endif
		ITEM_MANAGER::instance().RemoveItem(item);
							...
							if (item->GetValue(0) == AFFECT_LOOTING_SYSTEM)
							{
								...
								if (pkAff)
								{
									if (lDur > 0)
									{
										...
										{
											ChatPacket(CHAT_TYPE_NOTICE, "The duration of the affect cannot be increased any more.");
											return false;
										}
									}
									else
									...
									RemoveAffect(pkAff);
									ChatPacket(CHAT_TYPE_NOTICE, "Loot filter duration extended.");
								}
								else
								{
									ChatPacket(CHAT_TYPE_NOTICE, "Loot filter activated.");
								}
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, WORD count)
{
	...
	else
	{
		...
		if ((item2 = GetItem(DestCell)) && item->GetVnum() == item2->GetVnum() && item->GetType() == ITEM_BLEND && item2->GetType() == ITEM_BLEND && item != item2 && item->GetSocket(1) == item2->GetSocket(1))
		{
			if (item2 && !isPermamentBlend)
			{
			...
			else
			{
					...
					for (int i = 0; i < SPECIAL_STORAGE_INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							if (bCount == 0)
							{
								char szTemp[256];
								snprintf(szTemp, sizeof(szTemp), LC_TEXT(" x%d "), item->GetCount());
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You received: %s"), item2->GetName());	
								M2_DESTROY_ITEM(item);
...
#endif
				{
					WORD bCount = item->GetCount(); // change type for some
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							BYTE bCount2 = MIN(g_bItemCountLimit - item2->GetCount(), bCount); // change type for some
							bCount -= bCount2;
				...
				else if (item->IsSpecialStorageItem())
				{
					...
					if (iEmptyCell == -1)
					{
						...
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has no space, so the item %s%s moving automatically to inventory."), GetStorageName[GetSpecialWindowType], item->GetName(), (item->GetCount() > 1 ? szTemp : " "));
					...
					else if (iEmptyCell != -1)
					{
						char szTemp[256];
						snprintf(szTemp, sizeof(szTemp), LC_TEXT(" x%d "), item->GetCount());
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Hai ricevuto %s%s - puoi trovarlo nell'inventario speciale. [PREMI U]"), item->GetName(), (item->GetCount() > 1 ? szTemp : " "), GetStorageName[GetSpecialWindowType]);
				...
				if (item->IsDragonSoul()){
					...
					//ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Item is x."));
				}
#ifdef __SPECIAL_STORAGE_SYSTEM__
				else if (item->IsSpecialStorageItem())
					item->AddToCharacter(this, TItemPos(INVENTORY, iEmptyCell));
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(GetLanguage(),"Devi attendere un istante."));
		return false;
	}
...
void CHARACTER::RemoveSpecifyTypeItem(BYTE type, DWORD count)
{
	...
	for (int j = SKILLBOOK_INVENTORY; j <= GENERAL_INVENTORY; ++j) //through all types
	{
		for (int i = 0; i < SPECIAL_STORAGE_INVENTORY_MAX_NUM; ++i) //through all slots
		{
			...
			//�����ϩ�AI ��ioA�ˢ��?�ˢ� ��ii����I��iE ��o��������CAI�ˡ�e �ϩ�N��ui������I�ˡ�U. (�����ϩ�AI ��ioA�ˢ��?�ˢ硧u���� ����C�ˡ�A��iE��O���� AI ����I��������A�ˡ�����I ��ie��ui��?A �����Ϣ���?i ��o��cA|!)
			if(m_pkMyShop)
			{
				bool isItemSelling = m_pkMyShop->IsSellingItem(item->GetID());
				if (isItemSelling)
		...
		case DEVILTOWER_BLACKSMITH_ARMOR_MOB:
			{
				if (item->GetType() == ITEM_ARMOR &&
					...
					(item->GetSubType() == ARMOR_BODY || item->GetSubType() == ARMOR_SHIELD || item->GetSubType() ==
						ARMOR_HEAD || item->GetSubType() == ARMOR_PENDANT) &&
	#else
...
bool CHARACTER::IsItemEquip(DWORD item)
{
	for (int i = 0; i < WEAR_MAX_NUM; ++i)
	{
		auto pkItem = GetWear(i);
		if (pkItem && pkItem->GetVnum() == item)
			return true;
	}
...
void CHARACTER::RefreshPendantSet()
{
	if (!IsItemLoaded())
		return;
	...
	static std::vector<DWORD> itemSet = {19, 11299};
	for (const auto& item : itemSet)
		if (!IsItemEquip(item))
			return;

	AddAffect(AFFECT_EQUIPMENT_SET, POINT_ATTBONUS_MONSTER, 8, AFF_NONE, INFINITE_AFFECT_DURATION, 0, false, false);
}
