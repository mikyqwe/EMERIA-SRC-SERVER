				...
				if (ch->GetDesc())
				{
					...
					if (pTable)
					{
						...
						if (3==processReturn) //±³È¯Áß
							len = snprintf(buf, sizeof(buf), LC_TEXT_LANGUAGE(ch->GetLanguage(),"´Ù¸¥ °Å·¡Áß(Ã¢°í,±³È¯,»óÁ¡)¿¡´Â °³ÀÎ»óÁ¡À» »ç¿ëÇÒ ¼ö ¾ø½À´Ï´Ù."), pTable->szLocaleName[ch->GetLanguage()]);
						else
							len = snprintf(buf, sizeof(buf), LC_TEXT_LANGUAGE(ch->GetLanguage(),"%sÀÌ ÇÊ¿äÇÕ´Ï´Ù."), pTable->szLocaleName[ch->GetLanguage()]);
						#else
						if (3==processReturn) //±³È¯Áß
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (0!=processReturn)
	{
		...
		if (NULL != pTable)
		{
			#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
			if (3==processReturn) //±³È¯Áß
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"´Ù¸¥ °Å·¡Áß(Ã¢°í,±³È¯,»óÁ¡)¿¡´Â °³ÀÎ»óÁ¡À» »ç¿ëÇÒ ¼ö ¾ø½À´Ï´Ù."), pTable->szLocaleName[ch->GetLanguage()]);
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"%sÀÌ ÇÊ¿äÇÕ´Ï´Ù."), pTable->szLocaleName[ch->GetLanguage()]);
			#else
			if (3==processReturn) //±³È¯Áß
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"»ç¿ëÇÒ¼ö ¾ø½À´Ï´Ù."), pTable->szLocaleName);
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"%sÀÌ ÇÊ¿äÇÕ´Ï´Ù."), pTable->szLocaleName);
			#endif
		}
...
/* #ifdef ENABLE_CHAT_COLOR_SYSTEM
	// static const char* colorbuf[] = {" |h|r[aNC¨­?|cFFffa200|h", " |h|r[OiaO?|cFFff0000|h", " |h|r[O©¡aI?|cFFffc700|h", " |h|r[Iia©¡]|cFF000bff|h"}; // Arab
		...
		case SHOP_SUBHEADER_CG_SELL2:
			{
				...
				CShopManager::instance().Sell(ch, p2->bySlot, p2->byCount, p2->bType);
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	else if (pkItem->IsSpecialStorageItem())
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("L'oggetto %s%s non puo' essere riposto qui."), pkItem->GetName(), (pkItem->GetCount() > 1 ? szTemp : " "));
			return;
		}
		...
		if (iCell < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("#EMPTYSPECIALSTORAGETEXT"));
			return;
		}
...
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT `language` FROM account.account INNER JOIN player.player ON player.player.account_id = account.account.id WHERE player.player.`name` = '%s'", c_pszName));
...
std::string CInputMain::GetEmpireFlag(const char* c_pszName)
{
	...
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT `empire` FROM player.player_index INNER JOIN player.player ON player.player.account_id = player.player_index.id WHERE player.player.`name` = '%s'", newName));
	MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
	int iEmpireIndex = 0;
