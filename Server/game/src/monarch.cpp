...
struct FHealMyEmpire
{
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"군주가 내린 축복으로 모든 에너지가 가득 채워집니다"));
...
int CMonarch::HealMyEmpire(LPCHARACTER ch ,DWORD price)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"%d 초 후에 군주의 축복을 사용할 수 있습니다"), ch->GetMCLTime(CHARACTER::MI_HEAL));
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"국고에 돈이 부족합니다. 현재 : %u 필요금액 : %u"), GetMoney(Empire), price);
		return 0;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"[TEST_ONLY]현재 국고 : %d "), GetMoney(Empire) - price);
	return 1;
}
