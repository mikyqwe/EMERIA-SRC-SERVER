...
typedef struct character_point_instant
{
	...
	LPITEM			pSkillBookItems[SPECIAL_STORAGE_INVENTORY_MAX_NUM];
	BYTE			bSkillBookGrid[SPECIAL_STORAGE_INVENTORY_MAX_NUM];

	LPITEM			pUppItems[SPECIAL_STORAGE_INVENTORY_MAX_NUM];
	BYTE			bUppItemsGrid[SPECIAL_STORAGE_INVENTORY_MAX_NUM];

	LPITEM			pGhostStoneItems[SPECIAL_STORAGE_INVENTORY_MAX_NUM];
	BYTE			bGhostStoneGrid[SPECIAL_STORAGE_INVENTORY_MAX_NUM];

	LPITEM			pGeneralItems[SPECIAL_STORAGE_INVENTORY_MAX_NUM];
	BYTE			bGeneraGrid[SPECIAL_STORAGE_INVENTORY_MAX_NUM];
#endif
...
public:
	...
	public:
		...
		BYTE				GetLanguageForSearchShop() const;
		const char *		GetLanguageAsChar() const;
...
, bool bForceExp = false
	...
	public:
		...
		enum BioEnums
		{
			BIO_MAX_ITEM = 3, // always 3, length of the item array
			BIO_STATE_NOT_READY = 0, 
			BIO_STATE_1 = 0,
			BIO_STATE_2 = 1,
			BIO_STATE_REWARD = 2,
			BIO_LAST_MISSION = 10, // Last index in BioMissions (constans.h)
			BIO_REWARD_INDEX = 2,
			BIO_END_LEVEL = 96 + 1, // Last bio level and + 1
		} BioEnumData;

		int GetBioLevel() { return GetQuestFlag("bio.level"); }
		int GetBioCount() { return GetQuestFlag("bio.count"); }
		int GetBioState() { return GetQuestFlag("bio.state"); }
		int GetBioLastTry() { return GetQuestFlag("bio.trytime"); }

		void SetBioLevel(int level) { SetQuestFlag("bio.level", level); }
		void SetBioCount(int level) { SetQuestFlag("bio.count", level); }
		void SetBioState(int level) { SetQuestFlag("bio.state", level); }
		void SetBioLastTry(int level) { SetQuestFlag("bio.trytime", level); }


		void UpdateBioProcess(int slot);
		void SendBioUpdate();
		void SendBioData();
#endif
	...
	public:
		...
		LPITEM			GetSpecialStorageItem(WORD wCell, int invType) const;
#endif
