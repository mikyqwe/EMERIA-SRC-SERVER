...
void Cube_open (LPCHARACTER ch)
{
	...
	if (ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"이미 제조창이 열려있습니다."));
		return;
	}
#ifdef OFFLINE_SHOP
...
#endif
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"다른 거래중(창고,교환,상점)에는 사용할 수 없습니다."));
		return;
	}
...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (!(ch)->IsCubeOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE((ch)->GetLanguage(),"제조창이 열려있지 않습니다"));
		return false;
	}
	...
	if (NULL == cube_proto)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"제조 재료가 부족합니다"));
		return false;
	}
	int cube_gold = cube_proto->gold;
	if (cube_gold < 0 || ch->GetGold() < cube_gold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"돈이 부족하거나 아이템이 제자리에 없습니다."));	// 이 텍스트는 이미 널리 쓰이는거라 추가번역 필요 없음
		return false;
	}
	...
	{
		// 실패
		ch->ChatPacket(CHAT_TYPE_INFO, "Mi dispiace, ho fallito.");	// 2012.11.12 새로 추가된 메세지 (locale_string.txt 에 추가해야 함)
...
void Cube_show_list (LPCHARACTER ch)
{
	...
	for (int i=0; i<CUBE_MAX_NUM; ++i)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "cube[%d]: inventory[%d]: %s", i, item->GetCell(), item->GetName(ch->GetLanguage()));
	}
}
...
void Cube_add_item (LPCHARACTER ch, int cube_index, int inven_index)
{
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, "cube[%d]: inventory[%d]: %s added", cube_index, inven_index, item->GetName(ch->GetLanguage()));
...
void Cube_delete_item (LPCHARACTER ch, int cube_index)
{
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, "cube[%d]: cube[%d]: %s deleted", cube_index, item->GetCell(), item->GetName(ch->GetLanguage()));
