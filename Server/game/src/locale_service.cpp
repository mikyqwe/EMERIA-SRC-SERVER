...
void LocaleService_LoadLocaleStringFileNew()
{
	if (g_bAuthServer)
		return;
	...
	{
		std::string stLocaleStringFile = g_stServiceBasePath + "/translate/" + languages[i].c_str() + "/" + "locale_string.txt";

		fprintf(stderr, "Load LocaleString %s\n", stLocaleStringFile.c_str());
		locale_init_new(i, stLocaleStringFile.c_str());
	}
}
#endif
