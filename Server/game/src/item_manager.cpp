...
bool ITEM_MANAGER::Initialize(TItemTable * table, int size)
{
	...
	for (i = 0; i < size; ++i)
	{
		...
		len2 = snprintf(buf + len, sizeof(buf) - len, "%5u %-16s", m_vec_prototype[i].dwVnum, m_vec_prototype[i].szLocaleName[0]);
		#else
...
bool ITEM_MANAGER::GetVnum(const char * c_pszName, DWORD & r_dwVnum)
{
	...
	for (DWORD i = 0; i < m_vec_prototype.size(); ++i, ++pTable)
	{
		...
		{
			if (!strncasecmp(c_pszName, pTable->szLocaleName[j], len))
			{
