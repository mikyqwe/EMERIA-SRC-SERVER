...
ACMD(do_user_horse_back)
{
	...
	if (mount) {
		...
		if (ch->GetHorse() != NULL)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"말을 ?��?��보냈?��?��?��."));
		}
		else if (ch->IsHorseRiding() == true)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"말에?�� 먼�?? ?��?��?�� ?��?��?��."));
		}
		else
		{
...
ACMD(do_user_horse_feed)
{
	...
	if (ch->GetHorse() == NULL)
	{
		if (ch->IsHorseRiding() == false)
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"말을 ?�� ?��?��?��?��?�� 먹이�? �? ?�� ?��?��?��?��."));
		return;
	}
	...
	if (ch->CountSpecifyItem(dwFood) > 0)
	{
		...
		ch->FeedHorse();
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"말에�? %s%s 주었?��?��?��."),
	...
	{
		#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"%s ?��?��?��?�� ?��?��?��?��?��"), ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName[ch->GetLanguage()]);
		#else
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"%s ?��?��?��?�� ?��?��?��?��?��"), ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName);
		#endif
	}
}
	...
	if (subcmd != SCMD_RESTART_TOWN && (!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG))
	{
		if (!test_server)
		{
			if (ch->IsHack())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"?���? ?��?��?�� ?�� ?�� ?��?��?��?��. (%d�? ?��?��)"), iTimeToDead - (170 - g_nPortalLimitTime));
					return;
				}
			}
#define eFRS_HERESEC	175
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"?���? ?��?��?�� ?�� ?�� ?��?��?��?��. (%d�? ?��?��)"), iTimeToDead - eFRS_HERESEC);
				return;
			}
		}
	}
...
#define eFRS_TOWNSEC	180
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"?���? 마을?��?�� ?��?��?�� ?�� ?�� ?��?��?��?��. (%d �? ?��?��)"), iTimeToDead - eFRS_TOWNSEC);
			return;
		}
	}
	//END_PREVENT_HACK
...
ACMD(do_stat_minus)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"?���? 중에?�� ?��?��?�� ?���? ?�� ?��?��?��?��."));
		return;
	}
...
ACMD(do_guildskillup)
{
	...
	if (!ch->GetGuild())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<길드> 길드?�� ?��?��?���? ?��?��?��?��."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<길드> 길드 ?��?�� ?��벨을 �?경할 권한?�� ?��?��?��?��."));
	}
}
...
ACMD(do_safebox_change_password)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<창고> ?��못된 ?��?���? ?��?��?��?��?��?��?��."));
		return;
	}
...
ACMD(do_mall_password)
{
	...
	if (ch->GetMall())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<창고> 창고�? ?���? ?��?��?��?��?��?��."));
		return;
	}

	if (iPulse - ch->GetMallLoadTime() < passes_per_sec * 10) // 10초에 ?��번만 ?���? �??��
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<창고> 창고�? ?��???�? 10�? ?��?��?�� ?�� ?�� ?��?��?��?��."));
		return;
	}
...
ACMD(do_ungroup)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<?��?��> ?���? 문제�? ?��?�� �??�� 처리�? ?�� ?�� ?��?��?��?��."));
		return;
	}
...
ACMD(do_war)
{
	...
	switch (g->GetGuildWarState(opp_g->GetID()))
	{
		case GUILD_WAR_NONE:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> ?��???�? 길드�? ?���? ?��?�� �? ?��?��?��."));
					return;
				}
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> ?��비�?? �?족하?�� 길드?��?�� ?�� ?�� ?��?��?��?��."));
					return;
				}

				if (opp_g->GetGuildMoney() < iWarPrice)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> ?��???�? 길드?�� ?��비�?? �?족하?�� 길드?��?�� ?�� ?�� ?��?��?��?��."));
					return;
				}
			}
			break;
		...
		case GUILD_WAR_SEND_DECLARE:
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("?���? ?��?��?���? 중인 길드?��?��?��."));
				return;
			}
			break;
