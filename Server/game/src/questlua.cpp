	...
	#error "lua version not found"
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		{
			...
			for (int i = 0; i < _countof(languages); i++)
			{
				...
				snprintf(translateFileNameNew, sizeof(translateFileNameNew), "%s/translate/%s/translate.lua", LocaleService_GetBasePath().c_str(), languages[i].c_str());
				sys_log(0, "LoadTranslateMultiLang(%s), returns %d", translateFileNameNew, lua_dofile(L, translateFileNameNew));
