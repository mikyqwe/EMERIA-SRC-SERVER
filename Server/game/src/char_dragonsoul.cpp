...
bool CHARACTER::DragonSoul_ActivateDeck(int deck_idx)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(GetLanguage(),"용혼석 상자가 활성화되지 않았습니다."));
		return false;
	}

#ifdef ENABLE_DS_ACTIVE_LOOP
	AddAffect(AFFECT_DRAGON_SOUL_DECK_0 + deck_idx, APPLY_NONE, 0, AFF_DS_ACTIVE, INFINITE_AFFECT_DURATION, 0, false);
#else
