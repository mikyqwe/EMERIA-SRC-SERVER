...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"(%s) 길드가 생성되었습니다. [임시]"), cp.name);
}
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"%s: 존재하지 않는 길드 입니다."), arg1);
	}
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"그런 이름 또는 번호의 길드가 없습니다."));
		else
		{
			char buf[1024+1];
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"존재하지 않는 건물입니다."));
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"같이 지을 수 없는 종류의 건물이 지어져 있습니다."));
						return;
					}
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"건설에 필요한 건물이 지어져 있지 않습니다."));
							return;
						}
					}
				}
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"건물 비용 정보 이상으로 건설 작업에 실패했습니다."));
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"건설 비용이 부족합니다."));
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"자재가 부족하여 건설할 수 없습니다."));
							return;
						}
					}
				}
				...
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"건물을 지을 수 없는 위치입니다."));
					return;
				}
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"존재하지 않는 캐릭터 입니다."));
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"대련 강제 종료 실패"));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"대련 강제 종료 성공"));
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(pChar1->GetLanguage(),"<파티> 파티에서 나가셨습니다."));
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(pChar2->GetLanguage(),"<파티> 파티에서 나가셨습니다."));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"대련이 성공적으로 시작 되었습니다."));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"대련 시작에 문제가 있습니다."));
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"대련자가 없습니다."));
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"남은 스탯 포인트가 없습니다."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"남은 스탯 포인트가 적습니다."));
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"값을 잘못 입력하였습니다."));
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"명령어의 서브 커맨드가 잘못 되었습니다."));
			return;
			break;
	}
#endif
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"플레이어 %d 와 플레이어  %d를 파혼시킵니다.."), pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"각제국 진행 정보"));
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"선택 맵 정보 성지 %d 통로 %d %d %d"), GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3));
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"나의 삼거리 진행정보"));
