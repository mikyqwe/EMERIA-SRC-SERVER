...
namespace quest
{
	...
	ALUA(_item_name)
	{
		if (lua_isnumber(L,1))
		{
			...
			if (pTable)
				#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM	
				lua_pushstring(L,pTable->szLocaleName[ch ? ch->GetLanguage() : 0]);
				#else
	...
	ALUA(_mob_name)
	{
		if (lua_isnumber(L, 1))
		{
			...
			if (pkMob)
				#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
				lua_pushstring(L, pkMob->m_table.szLocaleName[ch ? ch->GetLanguage() : 0]);
				#else
...
#define NAN (INFINITY-INFINITY)
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushstring(L, (ch) ? ch->GetLanguageAsChar() : "en");
		return 1;
	}
#endif
