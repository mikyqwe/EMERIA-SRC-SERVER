...
int pTitleBonus_2[] = {POINT_ATTBONUS_MONSTER, 10};
int pTitleBonus_3[] = {POINT_CRITICAL_PCT, 10};
...
const char* requirements[] = {
									"[Onorificenze] Devi avere almeno %u yang per questo titolo!", 
									"[Onorificenze] Devi avere almeno un livello minimo di %u per questo titolo.",
									"[Title System] Devi avere i seguenti minuti giocati su questo personaggio: %u"};							
const char* title_translate[] = {
									"[Onorificenze] Devi aspettare %u ore per comprare una nuova pozione!",
									"[Onorificenze] Non hai abbastanza monete, contatta uno staffer per acquistare!",
									"[Onorificenze] Il tuo bonus e' stato rimosso!",
									"[Onorificenze] Il tuo titolo e' stato rimosso!, received x1 Pozione gialla incantata!",
									"[Onorificenze] Il tuo titolo e' stato rimosso!, received x1 Pozione rosa incantata!",							
									"[Onorificenze] Il tuo titolo e' stato rimosso!, Ricevi x1 Pozione bianca incantata!",
									"[Onorificenze] Non hai un titolo premium!",
									"[Onorificenze] Hai un titolo premium attivo, non puoi farlo!",
									"[Onorificenze] Hai gia' questo titolo!",
									"[Onorificenze] Devi avere una pozione gialla incantata!",
									"[Onorificenze] Devi avere una pozione rosa incantata!",
									"[Onorificenze] Devi avere una pozione bianca incantata!",
									"[Onorificenze] Il tuo titolo e' stato rimosso!"};						
...
*/
	...
	{	
		...
		if (x[0] < sPotionPrice[0])	{
			...
			ch->AutoGiveItem(sPotionVnum[0]);
			delete(DBManager::instance().DirectQuery("UPDATE account.account SET coins = coins - %u WHERE id = %u", sPotionPrice[0], ch->GetDesc()->GetAccountTable().id));
		#ifdef ENABLE_CHECK_TIME_POTIONS		
			pPC->SetFlag(sPotionFlag[0], get_global_time());
		#endif		
		
		#ifdef ENABLE_GIVE_JCOINS
			delete(DBManager::instance().DirectQuery("UPDATE account.account SET jcoins = jcoins + %u WHERE id = %u", sPotionPrice[0], ch->GetDesc()->GetAccountTable().id));	
		#endif			
		}
//}		
/****************
...
*/
	if (!strcmp(mPotion, "buy_potion_2"))
	{
		...
		if (x[1] < sPotionPrice[1])	{
			...
			ch->AutoGiveItem(sPotionVnum[1]);
			delete(DBManager::instance().DirectQuery("UPDATE account.account SET coins = coins - %u WHERE id = %u", sPotionPrice[1], ch->GetDesc()->GetAccountTable().id));
		#ifdef ENABLE_CHECK_TIME_POTIONS		
			pPC->SetFlag(sPotionFlag[1], get_global_time());
		#endif		
		
		#ifdef ENABLE_GIVE_JCOINS
			delete(DBManager::instance().DirectQuery("UPDATE account.account SET jcoins = jcoins + %u WHERE id = %u", sPotionPrice[1], ch->GetDesc()->GetAccountTable().id));	
		#endif		
	}	
//}	
/****************
...
*/	
	if (!strcmp(mPotion, "buy_potion_3"))
	{
		...
		if (x[2] < sPotionPrice[2])	{
			...
			ch->AutoGiveItem(sPotionVnum[2]);
			delete(DBManager::instance().DirectQuery("UPDATE account.account SET coins = coins - %u WHERE id = %u", sPotionPrice[2], ch->GetDesc()->GetAccountTable().id));
		#ifdef ENABLE_CHECK_TIME_POTIONS		
			pPC->SetFlag(sPotionFlag[0], get_global_time());
		#endif		
		
		#ifdef ENABLE_GIVE_JCOINS
			delete(DBManager::instance().DirectQuery("UPDATE account.account SET jcoins = jcoins + %u WHERE id = %u", sPotionPrice[2], ch->GetDesc()->GetAccountTable().id));	
		#endif	
	}
}	
