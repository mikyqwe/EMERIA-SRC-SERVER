...
namespace mining
{
...
#ifdef ENABLE_PICKAXE_RENEWAL
			{
				// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<Pickax> PRE %u"), Pick_GetCurExp(*item));
				Pick_SetPenaltyExp(*item);
				// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<Pickax> POST %u"), Pick_GetCurExp(*item));
				// ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"<Pickax> The upgrade has failed, and the pickax has lost 10%% of its mastery points."));
	...
	void CHEAT_MAX_PICK(LPCHARACTER ch, LPITEM item)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"곡괭이 수련도가 최대(%d)가 되었습니다."), Pick_GetCurExp(pick));
	}
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"곡괭이가 최대 수련도에 도달하였습니다."));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"나무꾼를 통해 다음 레벨의 곡괭이로 업그레이드 할 수 있습니다."));
			}
			else
			{
				Pick_IncCurExp(pick);

				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"곡괭이의 수련도가 증가하였습니다! (%d/%d)"),
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"곡괭이를 들고 있지 않아서 캘 수 없습니다."));
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"더이상 캐낼 수 없습니다."));
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"채광에 성공하였습니다."));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(ch->GetLanguage(),"채광에 실패하였습니다."));
		}
