...
CSwitchbot::CSwitchbot()
{
	...
	memset(&m_table, 0, sizeof(TSwitchbotTable));
	m_isWarping = false;
}
...
{
	for (int i = 0; i < _countof(m_table.active); ++i)
	{
		if (m_table.active[i])
		{
			return true;
	...
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		...
		else
		{
				...
				for (int i = 0; i < _countof(c_arSwitchingItems); ++i)
				{
					...
					if (itemVnum == 71151 || itemVnum == 76023)
					{
						if ((pkItem->GetType() == ITEM_WEAPON) || (pkItem->GetType() == ITEM_ARMOR))
						{
							...
							{
								if (pkItem->GetLimitType(i) == LIMIT_LEVEL && pkItem->GetLimitValue(i) > 50)
								{
									bCanUse = false;
									break;
								}
			...
			if (stop)
			{
				...
				if (SWITCHBOT_PRICE_TYPE == 1)
				{
					pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(pkOwner->GetLanguage(),"[S] Ho terminato gli incanta oggetto!"));
				}
				else
				{
					pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANGUAGE(pkOwner->GetLanguage(),"Switch stopped. Not enough money."));
				}
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				...
				{
					const DWORD& itemVnum = c_arSwitchingItems[i];
					LPITEM switcherItem = pkOwner->FindSpecifyItem(itemVnum);
					if (switcherItem && pkItem)
					{
						if (itemVnum == 71151 && switcherItem->GetSocket(0) != 0) {
							if (pkItem->GetType() == ITEM_WEAPON || pkItem->GetType() == ITEM_ARMOR) {
								bool skip = true;
								for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
								{
							...
							switcherItem->SetCount(switcherItem->GetCount() - SWITCHBOT_PRICE_AMOUNT);
							break;
						}
					}
				}
...
bool CSwitchbot::CheckItem(LPITEM pkItem, BYTE slot)
{
	...
	for (int i = 0; i < _countof(m_table.alternatives[slot]); ++i)
	{
		...
		for (int i = 0; i < _countof(alternative.attributes); ++i)
		{
			const TPlayerItemAttribute& destAttr = alternative.attributes[i];
